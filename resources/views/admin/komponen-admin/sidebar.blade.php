        ...
        <ul class="nav flex-column pt-3 pt-md-0">
            ...
            @foreach(\App\Helper\MenuSidebarHelper::generateMenuSidebar() as $data => $menu)
                @if(isset($menu['submenu']))
                    <li class="nav-item">
                        <span class="nav-link collapsed d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#submenu-{{ Str::slug($data) }}">
                            <span>
                                <span class="sidebar-icon"><i class="fas {{$menu['icon']}}"></i></span>
                                <span class="sidebar-text">{{$data}}</span>
                            </span>
                            <span class="link-arrow"></span>
                        </span>
                        <div class="multi-level collapse" role="list" id="submenu-{{ Str::slug($data) }}" aria-expanded="false">
                            <ul class="flex-column nav">
                                @foreach($menu['submenu'] as $submenu => $detail)
                                    <li class="nav-item">
                                        <a class="nav-link" href="{{$detail['url']}}">
                                            <span class="sidebar-icon"><i class="fas {{$detail['icon']}}"></i></span>
